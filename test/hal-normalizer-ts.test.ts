import normalize from "../src/hal-normalizer-ts"
...
describe('data is normalized', () => {
  ...
  it('data attributes', () => {
    const result = normalize(json, { normalizeUri: (uri: any) => uri.replace(/^http:\/\/www.example.com/, '') });

    expect(result).to.deep.equal(output);
  });

  it("data is empty shouldn't fail", () => {
    const result = normalize({}, { normalizeUri: (uri: any) => uri.replace(/^http:\/\/www.example.com/, '') });
...
describe('base URI removal', () => {
  ...
  it('removes prefix if option is set', () => {
    const result = normalize(json, { normalizeUri: (uri: any) => uri.replace(/^http:\/\/example.com\/api/, '') });
...
describe('URI handling', () => {
  it('normalizes link URIs', () => {
    ...
    const result = normalize(json, { normalizeUri: (uri: any) => uri.replace(/\?.*/, '') });
  ...
  it('can work with empty normalized URIs', () => {
    ...
    const result = normalize(json, { normalizeUri: (uri: any) => uri.replace(/^http:\/\/www.example.com\/api/, '') });
